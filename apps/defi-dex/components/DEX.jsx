...
import { useBalance, useContractReader } from '@drmg/shared/ui';
import { useTokenBalance } from '@drmg/shared/ui';
import { Address } from '@drmg/shared/ui';
// import { useTokenBalance } from 'eth-hooks/erc/erc-20/useTokenBalance';
// import { useEventListener } from "eth-hooks/events/useEventListener";
// import { useEventListener } from '@drmg/shared/ui';
...
export default function Dex(props) {
  ...
  const liquidity = useContractReader(
    props.readContracts,
    contractName,
    'totalLiquidity'
  );
      ...
      <div>
        {rowForm('ethToToken', '💸', async (value) => {
          ...
          let swapEthToTokenResult = await tx(
            writeContracts[contractName]['ethToToken']({ value: valueInEther })
          );
        ...
        {rowForm('tokenToEth', '🔏', async (value) => {
          ...
          if (allowance.lt(valueInEther)) {
            approveTx = await tx(
              writeContracts[tokenName].approve(
                props.readContracts[contractName].address,
                valueInEther,
                {
                  gasLimit: 200000,
          ...
          let swapTx = tx(
            writeContracts[contractName]['tokenToEth'](valueInEther, {
              gasLimit: 200000,
            })
          );
          if (approveTx) {
        ...
        <Divider>
          {' '}
          Liquidity ({liquidity ? ethers.utils.formatEther(liquidity) : 'none'}
          ):
        </Divider>
        ...
        {rowForm('deposit', '📥', async (value) => {
          ...
          await tx(
            writeContracts[contractName]['deposit']({
              value: valueInEther,
              gasLimit: 200000,
            })
          );
        ...
        {rowForm('withdraw', '📤', async (value) => {
          let valueInEther = ethers.utils.parseEther('' + value);
          let withdrawTxResult = await tx(
            writeContracts[contractName]['withdraw'](valueInEther)
          );
    ...
    <Row span={24}>
          ...
          title={
            <div>
              ...
              <div style={{ float: 'right', fontSize: 24 }}>
                {parseFloat(ethers.utils.formatEther(contractBalance)).toFixed(
                  4
                )}{' '}
                ⚖️
                <TokenBalance
      ...
      <Col span={12}>
        <div style={{ padding: 20 }}>
          <Curve
            addingEth={
              values && values['ethToToken'] ? values['ethToToken'] : 0
            }
            addingToken={
              values && values['tokenToEth'] ? values['tokenToEth'] : 0
            }
