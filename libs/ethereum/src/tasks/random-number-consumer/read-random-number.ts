...
task(
  ...
  .setAction(
    async (
      ...
      console.log(
        `Reading data from VRF contract ${contractAddr} on network ${networkId}`
      );
      ...
      try {
        const firstRandomNumber: BigNumber =
          await vrfConsumerContractV2.s_randomWords(0);
        const secondRandomNumber: BigNumber =
          await vrfConsumerContractV2.s_randomWords(1);
