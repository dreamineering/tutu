...
task('request-data', 'Calls an API Consumer Contract to request external data')
  ...
  .setAction(
    async (
      ...
      console.log(
        `Calling API Consumer contract ${contractAddr} on network ${hre.network.name}`
      );
      ...
      const apiConsumerContract: APIConsumer = APIConsumer__factory.connect(
        contractAddr,
        signer
      );

      const tx: ContractTransaction =
        await apiConsumerContract.requestVolumeData();
