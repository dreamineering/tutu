...
task('read-price-feed-ens', 'Gets the latest price from a Chainlink Price Feed')
  ...
  .setAction(
    async (
      ...
      console.log(
        `Reading data from Price Feed consumer contract ${ensAddress}`
      );
      ...
      const priceFeedConsumerContract: MockV3Aggregator =
        MockV3Aggregator__factory.connect(ensAddress, signer);
