...
task('withdraw-link', 'Returns any LINK left in deployed contract')
  ...
  .setAction(
    async (
      ...
      const balance: BigNumber = await linkTokenContract.balanceOf(
        contractAddr
      );

      console.log(
        `LINK balance of contract: ${contractAddr} is ${utils.formatEther(
          balance
        )}`
      );
      ...
      if (balance > constants.Zero) {
        ...
        const anyApiConsumerContract: APIConsumer =
          APIConsumer__factory.connect(contractAddr, signer);
        const transaction: ContractTransaction =
          await anyApiConsumerContract.withdrawLink();
