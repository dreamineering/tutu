...
export const getAccountData = async (
  mnemonic: string
): Promise<{ address: string; wallet: Wallet }> => {
  ...
  const address = `0x${privateToAddress(wallet.getPrivateKey()).toString(
    'hex'
  )}`;

  return { address, wallet: Wallet.fromMnemonic(mnemonic, fullPath) };
};
export const findFirstAddress = async (
...
export const getHardhatSigners = async (
  hre: THardhatRuntimeEnvironmentExtended
): Promise<THardhatAccounts> => {
