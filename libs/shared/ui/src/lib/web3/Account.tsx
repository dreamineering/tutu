...
import { useSignerAddress } from '@drmg/shared/ui';
import { TCreateEthersModalConnector } from '@drmg/shared/ui';
import { useEthersContext, useBlockNumberContext } from '@drmg/shared/ui';
  ...
  const handleLoginClick = (): void => {
    if (props.createLoginConnector != null) {
      ...
      if (!isMounted()) {
        ...
        invariant.warn(
          'openModal: A valid EthersModalConnector was not provided'
        );
      }
    }
  };
    ...
    <>
      {!showLoadModal && props.createLoginConnector && (
        <Button
          ...
          size="large"
          onClick={(): void =>
            ethersContext.disconnectModal(props.logoutOnSuccess)
          }
        >
          logout
        </Button>
